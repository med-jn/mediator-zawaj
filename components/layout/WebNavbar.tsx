...
import { Sun, Moon, LogOut, Wallet, Users, LayoutDashboard, ChevronDown } from 'lucide-react';
...
export function WebNavbar() {
  ...
  return (
    <nav style={{
      ...
      {/* Logo */}
      <Link href={isMediator ? '/agent' : '/mediators'} style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 8, marginInlineEnd: 'auto' }}>
        <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg, #800020, var(--color-primary))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 900, color: '#fff' }}>
          ز
        </div>
        <span style={{ fontSize: 15, fontWeight: 800, color: 'var(--text-main)' }}>زواج</span>
      </Link>

      {/* روابط المستخدم */}
      {user && !isMediator && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginInlineEnd: 12 }}>
          ...
          <div ref={dropdownRef} style={{ position: 'relative' }}>
            <button onClick={() => setShowDropdown(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 8px 4px 4px', borderRadius: 'var(--radius-full)', border: `1px solid ${bdr}`, background: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)', cursor: 'pointer' }}>
              {user.avatar_url && !avatarError
                ? <Image src={user.avatar_url} alt={user.full_name} width={28} height={28} style={{ borderRadius: '50%', objectFit: 'cover' }} onError={() => setAvatarError(true)} />
                : <AvatarFallback name={user.full_name} size={28} />}
              <ChevronDown size={13} style={{ color: 'var(--text-tertiary)', transform: showDropdown ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
            </button>
...
function NavLink({ href, active, icon, label }: { href: string; active: boolean; icon: React.ReactNode; label: string }) {
  return (
    <Link href={href} style={{ textDecoration: 'none' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: 'var(--radius-full)', background: active ? 'var(--color-primary-xsoft)' : 'transparent', border: `1px solid ${active ? 'var(--border-soft)' : 'transparent'}`, color: active ? 'var(--color-primary)' : 'var(--text-tertiary)', fontSize: 13, fontWeight: active ? 700 : 500, cursor: 'pointer' }}>
        {icon}{label}
      </div>
    </Link>
  );
